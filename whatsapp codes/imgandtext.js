...
client.on("message", async(message) => {
    ...
    if (lowerCaseMessage === "hello") {
    ...
    } else if (lowerCaseMessage === "test") {
        // Send a message with a heading, image, description, and a "More details" link
        const url =
            "https://cdn.britannica.com/84/73184-050-05ED59CB/Sunflower-field-Fargo-North-Dakota.jpg"; // Replace with your image URL
        ...
        try {
            ...
            const messageContent = `*Heading*\n\nThis is the description of the image. It provides more details about the content.\n\nFor more details, visit: https://example.com/more-details`;

            // Send the message content along with the image
            await client.sendMessage(message.from, media, {
                caption: messageContent,
            });
        } catch (error) {
            console.error("Failed to send the test message:", error);
            message.reply(
                "Sorry, I couldn't retrieve the content. Please try again later."
            );
        }
    } else {
